...
data Node = Node {pos :: Pos, size, used, avail :: Int} deriving (Show, Eq)
...
partTwo ns = fromMaybe 0 distToGoal + 5 * (maxX - 1) + 1 -- Each move toward the goal uses 5 steps + the final swap.
  where
    (maxX, maxY) = maximum $ map pos ns

    emptyNode = head [n | n <- ns, used n == 0]
    goal = (maxX - 1, 0)
    walls = Set.fromList [pos n | n <- ns, used n > size emptyNode]
    distToGoal = bfs (pos emptyNode) (maxX, maxY) goal walls

bfs :: Pos -> Pos -> Pos -> Set.Set Pos -> Maybe Int
bfs start (mx, my) goal walls = go (Seq.singleton (start, 0)) (Set.singleton start)
  where
    go Empty _ = Nothing
